...
app.get("/bears", (req, res) => {
  const leanBears = bears.map((bear) =>
    lodash.omit(bear, ["latinName", "habitat"])
  );
...
app.get("/bears/search", (req, res) => {
  const query = req.query.name || "";
  const lowerCaseQuery = query.toLowerCase();

  const leanBears = bears
    .filter(
      (bear) =>
        lowerCaseQuery && bear.name.toLowerCase().includes(lowerCaseQuery)
    )
    .map((bear) => lodash.omit(bear, ["latinName", "habitat"]));
  const randomDelay = generateRandomDelay();
...
app.listen(8080, () => console.log(`Bear API listening on port 8080!`));
